...
- Session on something else and the job needs more than one turn → mention once that `/model fable` keeps orchestrator mode across turns, or that `/orchestrate` can be re-run each turn.
...
| Renames, repetitive edits, scaffolding, formatting | `haiku` when it's truly mechanical, otherwise `sonnet` |
| Tests against a spec that already exists | `sonnet` |
| A first read-through of returned work | `Explore` (read-only), `opus` — the final verdict stays with Fable |
...
4. **Trust nothing unchecked.** Read the diffs. Run the build, the tests, the linter. A subagent reporting success is evidence, not proof, and passing that claim along as verified is the failure mode this whole mode exists to prevent.
