...
argument-hint: "[output-file] (default: cloudflare-zones-matrix.md)"
...
**Output file:** `$ARGUMENTS` — if empty, use `cloudflare-zones-matrix.md` in the current directory.
...
async () => {
  ...
  for (let i=0;i<zones.length;i+=limit){ const batch=zones.slice(i,i+limit); await Promise.all(batch.map(async z=>{ const rec={};
    ...
    try{const b=await cloudflare.request({method:"GET",path:`/zones/${z.id}/bot_management`});rec.bot_fight_mode=b.result?.fight_mode;}catch(e){rec.bot_fight_mode="n/a";}
  ...
  const CATS=[
    ...
    ["Security",["security_level","challenge_ttl","browser_check","bot_fight_mode","waf","privacy_pass","advanced_ddos","orange_to_orange"]],
